<div>
    ...
    <div class=" mt-1 mb-8 " id="sortable-item" wire:loading.class="hidden" wire:target="{{ $name }}">
        @if ($images_temp)
            @if (is_array($images_temp))
                @foreach ($images_temp as $key => $item)
                    <div
                        class="w-3/12 inline-block bg-white relative overflow-hidden rounded-lg ml-5 border border-gray-300">
                        <input class="images_order" wire:model.defer="images_order.image_tmp.{{ $key }}"
            ...
            @elseif($images_temp!="")
                <div
                    class="w-3/12 inline-block bg-white relative overflow-hidden rounded-lg ml-5 border border-gray-300">
                    <input class="images_order" wire:model.defer="images_order.image_tmp.1" value="" type="hidden">
                    <img src="{{ $images_temp->temporaryUrl() }}">
                </div>
        ...
        @if ($edit_var)
            @if ($multiple && $images_saved)
                ...
                @foreach ($images_saved as $key => $item)

                    <div wire:key="{{ $item->id }}"
                        class="w-3/12 inline-block bg-white relative overflow-hidden rounded-lg ml-5 border border-gray-300">

                        <input class="images_order" wire:model.defer="images_order.image_saved.{{ $item->id }}"
                            value="{{ $key }}" type="hidden">

                        <img src="{{ $path }}{{ $item->image }}?{{ rand(1, 300) }}">

                        <div x-data="{show:false}" class="text-center bg-red-700 text-white ">
                            <div class="cursor-pointer p-2 " @click="show=true">
                                <svg class="h-5 w-5  inline-block" xmlns="http://www.w3.org/2000/svg"
                            ...
                            <div x-show="show" class="text-left">
                                ...
                                <div
                                    class="absolute flex flex-col items-center inset-0  p-2 text-gray-500 text-sm justify-center ">
                                    <div wire:loading.remove wire:target="removeImg({{ $item->id }})"
                                    ...
                                    <div wire:loading wire:target="removeImg({{ $item->id }})">
                                        <svg class="animate-spin h-7 w-7 text-red-700"
